...
import { FaVideo, FaBook, FaUser, FaPlayCircle } from 'react-icons/fa';
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full bg-black border-t border-gray-800 pb-safe">
      <div className="flex justify-around py-3 px-2 max-w-md mx-auto">
        ...
        <Link href={user ? "/interactive-video" : "/login"} className={`flex flex-col items-center ${pathname === '/interactive-video' ? 'text-white' : 'text-gray-500'}`}>
          <div className={`p-1 rounded-full ${pathname === '/interactive-video' ? 'bg-green-600' : ''}`}>
            <FaPlayCircle className="text-xl mb-1" />
          </div>
          <span className="text-xs">Interaktywne</span>
        </Link>
